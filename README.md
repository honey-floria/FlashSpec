...
- microbenchmark 使用 CUDA event 记录 GPU latency，输出 `latency_p50_ms`、`latency_p90_ms`、raw measurements、`estimated_*` 和 `measured_*` 字段。KV bandwidth 仍是基于字节数和 latency 的估算值，JSON 中的 `bandwidth_fields_are_estimates` 会标记这一点；DRAM bytes、occupancy、SM utilization、registers/thread 和 theoretical occupancy 需要用 JSON 中的 Nsight Compute 命令采集后回填。用于内核实验的 `num_splits`、`block_n`、`env_flashspec_num_splits`、`env_flashspec_block_n` 也会写入 JSON，方便复核真实运行配置。
